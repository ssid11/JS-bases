// Мой код отмечен комментариями //Моя вставка
"use strict";
const settings = {
    rowsCount: 21,
    colsCount: 21,
        ...
        switch (this.direction) {
            case 'up':
                return {
                    x: firstPoint.x,
                    // Моя вставка
                    y: firstPoint.y > 0 ? firstPoint.y - 1 : config.settings.rowsCount - 1
                };
            case 'right':
                return {
                    // Моя вставка
                    x: firstPoint.x < config.settings.colsCount - 1 ? firstPoint.x + 1 : 0,
                    y: firstPoint.y
                };
            case 'down':
                return {
                    x: firstPoint.x,
                    // Моя вставка
                    y: firstPoint.y < config.settings.rowsCount - 1 ? firstPoint.y + 1 : 0
                };
            case 'left':
                return {
                    // Моя вставка
                    x: firstPoint.x > 0 ? firstPoint.x - 1 : config.settings.colsCount - 1,
                    y: firstPoint.y
                };
        }
    },
...
const game = {
    ...
    render() {
        // Моя вставка
        this.map.render(this.snake.body, this.food.getCoordinates(), this.wall.getCoordinates());
        this.countRef.innerText = `Текущий счет: ${this.snake.body.length}`
    },
    ...
    tickHandler() {
        ...
        if (this.wall.isOnPoint(this.snake.getNextHeadPoint())) return this.finish();
        if (this.food.isOnPoint(this.snake.getNextHeadPoint())) {
            this.snake.growUp();
            this.food.setCoordinates(this.getRandomFreeCoordinates());
            // Моя вставка
            if (this.snake.body.length % 5 === 0) this.wall.setCoordinates(this.getRandomFreeCoordinates());
